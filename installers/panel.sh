...
if [[ -z "${user_email}" ]]; then
  fail "User email is required"
  exit 1
fi

if [[ -z "${user_username}" ]]; then
  fail "User username is required"
  exit 1
fi

if [[ -z "${user_firstname}" ]]; then
  fail "User firstname is required"
  exit 1
fi

if [[ -z "${user_lastname}" ]]; then
  fail "User lastname is required"
  exit 1
fi

if [[ -z "${user_password}" ]]; then
  fail "User password is required"
  exit 1
fi
...
ptdl_dl() {
  log "Downloading Pterodactyl Panel files..."
  ...
  log "Downloading panel from: $PANEL_DL_URL"
  ...
  if ! tar -tzf panel.tar.gz >/dev/null 2>&1; then
    fail "Downloaded archive is corrupt"
    exit 1
  fi
  ...
  if [ -d "cons" ]; then
    log "Renaming 'cons' folder to 'config'..."
    mv cons config || {
      fail "Failed to rename 'cons' to 'config'"
      exit 1
    }
  fi
  ...
  if [ ! -f "artisan" ] || [ ! -d "app" ]; then
    fail "Panel files not properly extracted. Contents:"
    ls -la
    exit 1
  fi
  ...
  if [ ! -d "config" ]; then
    log "Config folder is missing, creating it."
    mkdir config || {
  ...
  confirm "Pterodactyl Panel confirmfully installed!"
...
install_composer_deps() {
  ...
  confirm "Installed composer dependencies!"
}
...
configure() {
  ...
  confirm "Configured environment!"
  log "Trying php cache and more"
  php artisan migrate && \
...
insert_cronjob() {
  log "Installing cronjob.. "

  crontab -l | {
    cat
    log "* * * * php /var/www/pterodactyl/artisan schedule:run >> /dev/null 2>&1"
  } | crontab -

  confirm "Cronjob installed!"
}

install_pteroq() {
  log "Installing pteroq service.."
...
firewall_ports() {
  log "Opening ports: 22 (SSH), 80 (HTTP) and 443 (HTTPS)"
...
configure_nginx() {
  ...
  confirm "Nginx configured!"
}

# --------------- Main functions --------------- #

perform_install() {
  log "Starting installation.. this might take a while!"
  dep_install
  install_composer
  ptdl_dl
  install_composer_deps
