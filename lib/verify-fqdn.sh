...
# exit with fail status code if user is not root
if [[ $EUID -ne 0 ]]; then
...
fail() {
  log "The DNS record ($dns_record) does not match your server IP. Please make sure the FQDN $fqdn is pointing to the IP of your server, $ip"
...
confirm() {
  log "This script will perform a HTTPS request to the endpoint $CHECKIP_URL"
  log "The official check-IP service for this script, https://checkip.pterodactyl-installer.se"
  log "- will not log or share any IP-information with any third-party."
  log "If you would like to use another service, feel free to modify the script."

  echo -e -n "* I agree that this HTTPS request is performed (y/N): "
  read -r confirm
  [[ "$confirm" =~ [Yy] ]] || (fail "User did not agree" && false)
}

dns_verify() {
  log "Resolving DNS for $fqdn"
